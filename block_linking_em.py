...
def compute_all_haplotypes(full_samples_data,
    ...
    for i in range(7):
        ...
        mesh = generate_transition_probability_mesh(full_samples_data,
...
#%%
test = explain_sample_viterbi([all_likelihoods[0][188],all_likelihoods[1][25]],all_sites,
                              test_haps,[found_haplotypes[0][:6]])
print(test)
#%%
space_gap = 1
...
check = found_haplotypes[0][6]
for i in range(len(haplotype_data)):
    print(i,analysis_utils.calc_perc_difference(check[1],
        haplotype_data[i][:37051],calc_type="haploid"))
    print(len(np.where(
        concretify_haps([check[1]])[0] !=
        concretify_haps([haplotype_data[i][:37051]])[0])[0]))
    print(len(check[1]))
    print()
#%%
for i in range(len(test_haps)):
    ...
    real_data_here = analysis_utils.get_sample_data_at_sites(
        haplotype_data[2],all_sites,sites_here)
    
    block_used_here = test_haps[i][3][new_haplotype[2][i]]
