...
def recombine_haps(hap_pair, ancestry_pair, site_locs,
    ...
    Args:
        ...
        rng: numpy random generator (for reproducibility in parallel)
    """
    if rng is None:
...
def _process_contig_for_generation(args):
    ...
    Args:
        args: tuple of (contig_idx, contig_data_dict)
            contig_data_dict contains:
                - 'parents': list of parent hap pairs for this contig
                - 'ancestries': list of parent ancestry pairs for this contig
                - 'site_locs': SNP positions for this contig
                - 'offspring_parent_indices': list of (p1_idx, p2_idx) for each offspring
                - 'recomb_rate': recombination rate
                - 'mutate_rate': mutation rate
                - 'seed': random seed for this contig
    
    Returns:
        (contig_idx, offspring_haps_list, offspring_ancestries_list)
    """
    contig_idx = args[0]
    data = args[1]
    ...
    # Create RNG with unique seed for this contig
...
def simulate_pedigree(founders, site_locs, generation_sizes, 
    ...
    Args:
        founders: List of founder individuals, or list of lists for multi-contig
        site_locs: SNP positions, or list of positions for multi-contig
        generation_sizes: List of offspring counts per generation
        recomb_rate: Per-bp recombination rate
        mutate_rate: Per-bp mutation rate
        output_plot: Path to save pedigree visualization
        max_workers: Number of parallel workers (default: number of contigs)
    ...
    if is_multi_mode:
        ...
        site_locs_list = [site_locs]
        num_contigs = 1
        parallel = False  # No point parallelizing single contig
    ...
    # Master RNG for generating seeds
