...
# ----------------------------------------------------------------------------
# SELF-CONTAINED snapshot of pipeline.py's setup + stages 2-11, parameterized
# for the read-depth x seed sweep. This file has no runtime dependency on
# pipeline.py. Its terminal pedigree stage uses the default current B1 engine
# and exports scientific, complete, Tier-B, and diagnostic views separately;
# no phase-correction stage follows it.
#
...
# symlinks the existing .pipeline_checkpoints_reversible_cavity_depth_observation_v1/01_founder_discovery in, so stage 1
...
_BHD_CKPT_DIR   = _bhd_os.environ.get("BHD_SWEEP_CKPT_DIR",
    "pedigree_sweep_checkpoints_reversible_cavity_depth_observation_v1")
_BHD_OUTPUT_DIR = _bhd_os.environ.get("BHD_SWEEP_OUTPUT_DIR",
    "pedigree_sweep_results_reversible_cavity_depth_observation_v1")
...
if __name__ == '__main__':
    ...
    #
    # For a fresh run, configure a new exact checkpoint directory.
...
if __name__ == '__main__':
    ...
    if stage_complete(STAGE_2):
        ...
        if (
            g.get('stage1_backend') != block_haplotypes.STAGE1_BACKEND
            or g.get('stage1_config') != stage1_config_record
        ):
            raise RuntimeError(
                "Stage-2 checkpoint was generated from an incompatible "
                "founder-discovery model"
            )
...
if __name__ == '__main__':
    # =============================================================================
    # STAGE 11: CALIBRATED COMBINED-V1 B1 PEDIGREE INFERENCE
    # =============================================================================
    ...
    if stage_complete(STAGE_11) and not checkpoint_store.global_done(STAGE_11):
        raise RuntimeError(f"{STAGE_11} is complete but lacks _global")
    if stage_complete(STAGE_11):
        print(f"\n[RESUME] Skipping current B1 pedigree inference (checkpoint found)")
        pedigree_payload = load_global(STAGE_11)
        pipeline_control_pedigree_df = pedigree_payload[
        ...
        for label, frame in (
            ('pipeline_control', pipeline_control_pedigree_df),
        ...
        for r_name in region_keys:
            _ensure_key(r_name, 'tolerance_result')
            _ensure_key(r_name, 'super_blocks_L4')
            if 'tolerance_result' not in multi_contig_results[r_name]:
        ...
        # The current engine copies each bundled founder block into shared
        # memory, so release checkpoint-backed duplicates before dispatch.
        for r_name in region_keys:
            multi_contig_results[r_name].pop('super_blocks_L4', None)
        ...
        pipeline_control_pedigree_df = pedigree_result.pipeline_control_relationships
        for label, frame in (
            ('scientific', scientific_pedigree_df),
            ('complete', complete_pedigree_df),
            ('tier_b', tier_b_pedigree_df),
        ...
        if 'truth_pedigree' in dir():
            ...
            for view_name, inferred_frame in (
                ...
                descendant_mask = validation_df["TruthGeneration"].isin(
                    ["F2", "F3"]
                )
                state_acc = (
                    validation_df.loc[
                        descendant_mask, "ParentState_Match"
                    ].mean() * 100
                )
                parent_acc = (
                    validation_df.loc[
                        descendant_mask, "Parents_Match"
                    ].mean() * 100
                )
        ...
        save_global(STAGE_11, {
            'pipeline_control_relationships': pipeline_control_pedigree_df,
        ...
        if not checkpoint_store.global_done(STAGE_11):
            raise OSError(f"Failed to checkpoint {STAGE_11}/_global")
        del contig_inputs
        gc.collect()
