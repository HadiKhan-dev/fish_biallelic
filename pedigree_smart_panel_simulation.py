"""Paired known-truth validation across nested Smart candidate panels.
...
Every arm retains the same physical universe of 32 candidates and the same
truth, founders, true-core candidates, external parents, children, and child
observations. Arms differ only in candidate-relatedness decoys and in nested
eligible panels K=8,16,32, with every eligible opposite-sex pair evaluated.
Panel size never enters a genetics or observation RNG namespace.
"""
...
SCHEMA_VERSION = 3
ATTEMPT_IDENTITY = "pedigree_smart_nested_panel_combined_v1_known_truth_v3"
BASE_READ_ERROR = 0.02
ELEVATED_READ_ERROR = 0.05
N_FOUNDERS = 8
N_CANDIDATES = 32
FEMALE_CANDIDATES = tuple(range(16))
MALE_CANDIDATES = tuple(range(16, 32))
ELIGIBLE_CANDIDATE_COUNTS = (8, 16, 32)
CANDIDATE_RELATEDNESS_PROFILES = (
    "unrelated",
    "full_half",
    "full_half_near",
)
...
TRUTH_STRATA = (
    ...
    ("m1_low_parent", 1, (1,), (1,)),
    ("m2_normal", 2, (0, 16), ()),
    ("m2_one_low_parent", 2, (0, 17), (17,)),
    ("m2_both_low_parents", 2, (1, 17), (1, 17)),
)
...
CORE_RELATIONSHIP_LABELS = {
    ...
    1: "female_low_depth_truth_core",
    16: "male_normal_truth_core_anchor",
    17: "male_low_depth_truth_core",
}
...
def _parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        default=",".join(map(str, ELIGIBLE_CANDIDATE_COUNTS)),
    )
    parser.add_argument(
        "--candidate-relatedness",
        default=",".join(CANDIDATE_RELATEDNESS_PROFILES),
    )
    parser.add_argument(
        "--state-profile", choices=tuple(STATE_PROFILE_COUNTS), default="mixed"
    )
    ...
    parser.add_argument(
        ...
        default=12,
        help=(
            "recycle each outer worker after this many units to bound "
            "retained Numba/inference memory"
        ),
    )
...
def _validated_args(args):
    ...
    try:
        eligible_counts = tuple(
            int(value) for value in _comma_values(args.eligible_candidate_counts)
        )
    except ValueError as exc:
        raise ValueError(
            "eligible-candidate-counts must be comma-separated integers"
        ) from exc
    if (
        not eligible_counts
        or len(eligible_counts) != len(set(eligible_counts))
        or any(value not in ELIGIBLE_CANDIDATE_COUNTS for value in eligible_counts)
    ):
        raise ValueError("eligible candidate counts must be unique members of 8,16,32")
    candidate_relatedness = _comma_values(args.candidate_relatedness)
    unknown_relatedness = sorted(
        set(candidate_relatedness) - set(CANDIDATE_RELATEDNESS_PROFILES)
    )
    if (
        not candidate_relatedness
        or len(candidate_relatedness) != len(set(candidate_relatedness))
        or unknown_relatedness
    ):
        raise ValueError(
            "invalid candidate-relatedness list; "
            f"unknown={unknown_relatedness}"
        )
    try:
    ...
    if not 1 <= args.max_tasks_per_child <= 10_000:
        raise ValueError("max-tasks-per-child must lie in [1,10000]")
...
def _relationship_labels(candidate_relatedness):
    labels = {
        index: CORE_RELATIONSHIP_LABELS.get(index, "unrelated_decoy")
        for index in range(N_CANDIDATES)
    }
    if candidate_relatedness in ("full_half", "full_half_near"):
        labels.update({
    ...
    if candidate_relatedness == "full_half_near":
        labels.update({
...
def _candidate_universe(
    ...
    candidates = [
        _mosaic(
            founders,
            _rng(
                seed, split, "unrelated_candidate", replicate, contig, index
            ),
        )
        for index in range(N_CANDIDATES)
    ]
    anchor_context = {}
    for anchor, low_depth_core in ((0, 1), (16, 17)):
        core_rng = _rng(
            seed, split, "true_core_anchor", replicate, contig, anchor
        )
        first_grandparent = _mosaic(founders, core_rng)
        second_grandparent = _mosaic(founders, core_rng)
        other_grandparent = _mosaic(founders, core_rng)
        candidates[anchor] = _offspring(
            first_grandparent, second_grandparent, core_rng
        )
        candidates[low_depth_core] = _mosaic(
            founders,
            _rng(
                seed,
                split,
    ...
    if candidate_relatedness in ("full_half", "full_half_near"):
        for anchor, full_sibling, half_sibling in (
            (0, 2, 3),
            (16, 18, 19),
        ):
            first, second, other = anchor_context[anchor]
            candidates[full_sibling] = _offspring(
                first,
                second,
                _rng(
                    seed,
            ...
            candidates[half_sibling] = _offspring(
                first,
                other,
                _rng(
                    seed,
    ...
    if candidate_relatedness == "full_half_near":
        for anchor, near_duplicate in ((0, 4), (16, 20)):
            candidates[near_duplicate] = _near_duplicate(
                candidates[anchor],
                founders,
                _rng(
                    seed,
    ...
    return candidates, _relationship_labels(candidate_relatedness)
...
def _genetics(
    ...
    founders = _founders(
        _rng(seed, split, "founders", replicate, contig),
        n_bins,
        n_snps,
    )
    candidates, relationship_labels = _candidate_universe(
        founders,
        seed,
        split,
        replicate,
    ...
    for row in truth_rows:
        ...
        if family not in external_by_family:
            external_by_family[family] = [
                ...
                for external_index in range(2 - row["true_state"])
            ]
    ...
    return founders, alleles, labels, relationship_labels
...
def _observations(
    ...
    depth = np.empty(dosage.shape, dtype=np.int32)
    alt = np.empty(dosage.shape, dtype=np.int32)
    forced_zero = np.zeros(dosage.shape, dtype=np.bool_)
    error = (
        ELEVATED_READ_ERROR
        if scenario == "elevated_error_combined"
        else BASE_READ_ERROR
    )
    shared_rng = _rng(
        seed, split, "observation_shared", replicate, scenario, contig
    )
    site_multiplier = None
    shared_gap = None
    shared_site_dropout = None
    if scenario in ("heterogeneous_sample_site", "elevated_error_combined"):
        site_multiplier = shared_rng.lognormal(
            -0.32, 0.8, (n_bins, n_snps)
        )
        if scenario == "heterogeneous_sample_site":
            shared_site_dropout = (
                shared_rng.random((n_bins, n_snps)) < 0.08
            )
    if scenario == "contiguous_gaps":
        length = max(1, n_bins // 4)
        shared_gap = (max(0, n_bins // 3), length)
    elif scenario == "elevated_error_combined":
        length = max(1, n_bins // 5)
        shared_gap = (
            int(shared_rng.integers(max(1, n_bins - length + 1))),
            length,
        )

    low_depth_set = set(low_depth_candidates)
    for sample in range(n_samples):
        ...
        sample_mean = np.full((n_bins, n_snps), 8.0, dtype=np.float64)
        sample_forced_zero = forced_zero[sample]
        if scenario == "clean":
            sample_mean.fill(10.0)
        elif scenario == "mcar_2x":
            sample_mean.fill(2.0)
            sample_forced_zero |= rng.random((n_bins, n_snps)) < 0.15
        elif scenario == "heterogeneous_sample_site":
            sample_multiplier = float(rng.lognormal(-0.32, 0.8))
            sample_mean = np.clip(
                2.0 * sample_multiplier * site_multiplier,
                0.02,
                30.0,
            )
            sample_forced_zero |= shared_site_dropout
        elif scenario == "contiguous_gaps":
            sample_mean.fill(6.0)
            shared_start, length = shared_gap
            sample_forced_zero[shared_start:shared_start + length] = True
            start = int(rng.integers(max(1, n_bins - length + 1)))
            sample_forced_zero[start:start + length] = True
        elif scenario == "mnar_dropout":
            sample_mean.fill(4.0)
            sample_forced_zero |= rng.random((n_bins, n_snps)) < (
                0.05 + 0.35 * dosage[sample] / 2.0
            )
        elif scenario == "elevated_error_combined":
            sample_multiplier = float(rng.lognormal(-0.32, 0.8))
            sample_mean = np.clip(
            ...
            sample_forced_zero |= rng.random((n_bins, n_snps)) < (
                0.08 + 0.22 * dosage[sample] / 2.0
            )
            start, length = shared_gap
            sample_forced_zero[start:start + length] = True
        elif scenario == "low_child":
            sample_mean.fill(0.25 if sample >= N_CANDIDATES else 4.0)
        else:
            raise ValueError(f"unknown scenario {scenario!r}")

        if sample in low_depth_set:
            sample_mean.fill(low_parent_depth)
        sample_depth = rng.poisson(sample_mean).astype(np.int32)
        sample_depth[sample_forced_zero] = 0
        depth[sample] = sample_depth
        alt_probability = np.where(
            dosage[sample] == 0,
            error,
            np.where(dosage[sample] == 1, 0.5, 1.0 - error),
        )
        alt[sample] = rng.binomial(sample_depth, alt_probability).astype(
            np.int32
        )
...
def _eligible_candidates(
    ...
    half = eligible_candidate_count // 2
    if candidate_relatedness == "full_half_near":
        female_order = (0, 1, 2, 4, 3, *range(5, 16))
        male_order = (16, 17, 18, 20, 19, *range(21, 32))
    else:
        female_order = FEMALE_CANDIDATES
        male_order = MALE_CANDIDATES
    return female_order[:half], male_order[:half]


def _trios_and_eligibility(
    sample_ids,
    truth_rows,
    eligible_candidate_count,
    candidate_relatedness,
):
    eligible_females, eligible_males = _eligible_candidates(
        eligible_candidate_count,
        candidate_relatedness,
    )
    eligible_indices = (*eligible_females, *eligible_males)
    ...
    for truth in truth_rows:
        child = truth["child"]
        children[child] = True
        parents[child, np.asarray(eligible_indices, dtype=np.int64)] = True
    ...
    pair_count = len(eligible_females) * len(eligible_males)
    eligibility = smart.ParentEligibility(
        ...
        policy_name=(
            f"simulated_nested_{eligible_candidate_count}_candidate_panel_v1"
        ),
        source_fields=("simulated_candidate_role", "simulated_sex"),
        assumptions=(
            f"{eligible_candidate_count} nested candidates are eligible for M1",
            f"M2 enumerates all {pair_count} eligible opposite-sex pairs",
            "no lower-bound candidate screen is applied",
            "eligibility is not individual parentage truth",
        ),
        individual_parentage_ground_truth=False,
    )
    return (
        np.ascontiguousarray(trios, dtype=np.int64),
        eligibility,
        pair_count,
    )
...
def _unit_payload(spec):
    ...
    trios, eligibility, eligible_pair_count = _trios_and_eligibility(
        sample_ids,
        truth_rows,
        spec["eligible_candidate_count"],
        spec["candidate_relatedness"],
    )
    ...
    error_probabilities = []
    truth_sha256 = _canonical_hash(truth_rows)
    child_genetics_digest = hashlib.sha256()
    child_observations_digest = hashlib.sha256()
    candidate_universe_digest = hashlib.sha256()
    physical_observations_digest = hashlib.sha256()
    relationship_labels = None

    for contig in range(spec["contigs"]):
        founders, alleles, labels, relationship_labels = _genetics(
        ...
        for name, value in (
            ("candidate_alleles", alleles[:N_CANDIDATES]),
            ("candidate_labels", labels[:N_CANDIDATES]),
        ):
            _update_array_hash(
                candidate_universe_digest, f"{contig}:{name}", value
            )
        for name, value in (
            ("child_alleles", alleles[N_CANDIDATES:]),
            ("child_labels", labels[N_CANDIDATES:]),
        ):
            _update_array_hash(child_genetics_digest, f"{contig}:{name}", value)
        for name, value in (
            ("child_gl", gl[N_CANDIDATES:]),
            ("child_hard", hard[N_CANDIDATES:]),
            ("child_masked_labels", masked_labels[N_CANDIDATES:]),
            ("child_hom_reset", hom_reset_mask[N_CANDIDATES:]),
            ("child_depth", depth[N_CANDIDATES:]),
            ("child_wholly_missing", wholly_missing_bin[N_CANDIDATES:]),
        ):
            _update_array_hash(
                child_observations_digest, f"{contig}:{name}", value
            )
        for name, value in (
            ("physical_gl", gl),
            ("physical_hard", hard),
            ("physical_masked_labels", masked_labels),
            ("physical_hom_reset", hom_reset_mask),
            ("physical_depth", depth),
            ("physical_wholly_missing", wholly_missing_bin),
        ):
            _update_array_hash(
                physical_observations_digest, f"{contig}:{name}", value
            )
        theta = 0.5 * (
    ...
    child_genetics_sha256 = child_genetics_digest.hexdigest()
    child_observations_sha256 = child_observations_digest.hexdigest()
    candidate_universe_sha256 = candidate_universe_digest.hexdigest()
    physical_observations_sha256 = physical_observations_digest.hexdigest()
    ...
    for truth in truth_rows:
        ...
        record = {
            ...
            "candidate_relatedness": spec["candidate_relatedness"],
            "truth_sha256": truth_sha256,
            "child_genetics_sha256": child_genetics_sha256,
            "child_observations_sha256": child_observations_sha256,
            "candidate_universe_sha256": candidate_universe_sha256,
            "physical_observations_sha256": physical_observations_sha256,
    ...
    return {
        ...
        "cohort_diagnostics": {
            ...
            "eligible_candidate_count": spec["eligible_candidate_count"],
            "candidate_relatedness": spec["candidate_relatedness"],
            "eligible_pairs_per_child": eligible_pair_count,
            "trio_rows": len(trios),
...
def _run_and_checkpoint_tagged(spec):
    return spec, _run_and_checkpoint(spec)
...
def _unit_name(
    ...
    return (
        f"{split}.{scenario}.k{eligible_candidate_count}."
        f"{candidate_relatedness}.e{marker}.replicate-{replicate:07d}.json"
    )
...
def _metric_row(
    ...
    output = {
        ...
        "candidate_relatedness": candidate_relatedness,
...
def _summaries(
    ...
    for scenario in scenarios:
        for eligible_candidate_count in eligible_candidate_counts:
            for candidate_relatedness in candidate_relatedness_profiles:
                for effective in effective_markers:
                    cell = [
                        row for row in records
                        if row["scenario"] == scenario
                        and row["eligible_candidate_count"]
                        == eligible_candidate_count
                        and row["candidate_relatedness"]
                        == candidate_relatedness
                        and row["effective_markers"] == effective
                    ]
                    aggregate.append(_metric_row(
                        cell,
                        scenario,
                        eligible_candidate_count,
                        candidate_relatedness,
                    ...
                    for truth_state in range(3):
                        truth = [
                            row for row in cell
                            if row["truth_state"] == truth_state
                        ]
...
def _manifest(
    ...
    return {
        ...
        "interpretation": (
            "Paired known-truth validation of nested eligible panels through "
            "the current Smart raw-GL scorer and hierarchical prior, DAG, "
            "bootstrap, LOCO, and Tier A/B release engine."
        ),
        "parameters": {
            "seed": args.seed,
            "split": args.split,
            ...
            "eligible_candidate_counts": list(eligible_candidate_counts),
            "candidate_relatedness_profiles": list(
                candidate_relatedness_profiles
            ),
        ...
        "fixed_design": {
            ...
            "physical_candidate_universe": N_CANDIDATES,
            "candidate_females": len(FEMALE_CANDIDATES),
            "candidate_males": len(MALE_CANDIDATES),
            "nested_eligible_candidate_counts": list(eligible_candidate_counts),
            "eligible_pair_counts": {
                str(value): (value // 2) ** 2
                for value in eligible_candidate_counts
            },
            "candidate_order_by_relatedness": {
                profile: {
                    "female": list(_eligible_candidates(32, profile)[0]),
                    "male": list(_eligible_candidates(32, profile)[1]),
                }
                for profile in candidate_relatedness_profiles
            },
            "state_profile": args.state_profile,
            ...
            "true_core_candidates": {
                str(index): label
                for index, label in CORE_RELATIONSHIP_LABELS.items()
            },
            "candidate_relationships_by_profile": {
                profile: {
                    str(index): label
                    for index, label in _relationship_labels(profile).items()
                }
                for profile in candidate_relatedness_profiles
            },
        ...
        "rng_namespaces": {
            "founders": "(seed, split, founders, replicate, contig)",
            "true_core_anchor": "(..., true_core_anchor, replicate, contig, anchor)",
            "true_core_low_depth": "(..., true_core_low_depth, replicate, contig, index)",
            "unrelated_candidate": "(..., unrelated_candidate, replicate, contig, index)",
            "related_decoys": "(..., related_decoy_KIND, replicate, contig, anchor)",
            "external_parent": "(..., external_parent, replicate, contig, family, index)",
            "child": "(..., child, replicate, contig, family, sibling)",
            "observation_shared": "(..., observation_shared, replicate, scenario, contig)",
            "observation_sample": "(..., observation_sample, replicate, scenario, contig, sample)",
            "bootstrap": "(seed, split, bootstrap, replicate, scenario)",
            "paired_arm_exclusion": (
                "eligible panel K and candidate-relatedness profile are absent "
                "from founder, core, external, child, and observation namespaces"
            ),
        ...
        "resources": {
            ...
            "max_tasks_per_child": args.max_tasks_per_child,
            "worker_recycling_reason": (
                "bound retained Numba and inference memory across units"
            ),
        },
...
def _validate_paired_hashes(
    records,
    eligible_candidate_counts,
    candidate_relatedness_profiles,
):
    expected_arms = {
        (eligible_candidate_count, candidate_relatedness)
        for eligible_candidate_count in eligible_candidate_counts
        for candidate_relatedness in candidate_relatedness_profiles
    }
    grouped = {}
    for row in records:
        key = (
            ...
            row["replicate"],
            row["scenario"],
            row["effective_markers"],
            row["state_profile"],
            row["low_parent_depth"],
        )
        arm = (
            row["eligible_candidate_count"],
            row["candidate_relatedness"],
        )
        grouped.setdefault(key, {})[arm] = {
            "truth_sha256": row["truth_sha256"],
            "child_genetics_sha256": row["child_genetics_sha256"],
            "child_observations_sha256": row["child_observations_sha256"],
            "candidate_universe_sha256": row["candidate_universe_sha256"],
            "physical_observations_sha256": row["physical_observations_sha256"],
        }

    validations = []
    for key, arms in sorted(grouped.items(), key=lambda item: str(item[0])):
        if set(arms) != expected_arms:
            missing = sorted(expected_arms - set(arms))
            raise ValueError(f"paired arm set incomplete for {key}: missing={missing}")
        hashes = {}
        for field in (
            "truth_sha256",
            ...
            values = {value[field] for value in arms.values()}
            if len(values) != 1:
                raise ValueError(
                    f"paired {field} mismatch for {key}: {sorted(values)}"
                )
            hashes[field] = next(iter(values))
        nested_k_hashes = {}
        for candidate_relatedness in candidate_relatedness_profiles:
            related_arms = [
                value for arm, value in arms.items()
                if arm[1] == candidate_relatedness
            ]
            nested_k_hashes[candidate_relatedness] = {}
            for field in (
                "candidate_universe_sha256",
                "physical_observations_sha256",
            ):
                values = {value[field] for value in related_arms}
                if len(values) != 1:
                    raise ValueError(
                        f"nested-K {field} mismatch for {key}, "
                        f"relatedness={candidate_relatedness}"
                    )
                nested_k_hashes[candidate_relatedness][field] = next(iter(values))
        validations.append({
            "split": key[0],
            "replicate": key[1],
            "scenario": key[2],
            "effective_markers": key[3],
            "state_profile": key[4],
            "low_parent_depth": key[5],
            "arm_count": len(arms),
            **hashes,
            "nested_k_hashes_by_relatedness": nested_k_hashes,
        })
    return validations
...
def run(args):
    ...
    if maximum_workers == 1:
        for index, spec in enumerate(specs, start=1):
            ...
            print(
                ...
                f"K={spec['eligible_candidate_count']} "
                f"rel={spec['candidate_relatedness']} "
        ...
        with context.Pool(
            ...
            tagged_units = pool.imap_unordered(
                _run_and_checkpoint_tagged,
                specs,
                chunksize=1,
            )
            for spec, unit in tagged_units:
                records.extend(unit["records"])
                computed += 1
                print(
    ...
    records.sort(key=lambda row: (
        row["replicate"], row["scenario"],
        row["eligible_candidate_count"], row["candidate_relatedness"],
        row["effective_markers"],
        row["family"], row["within_family"],
    ))
    aggregate, confusion = _summaries(
        records,
        scenarios,
        eligible_candidate_counts,
        candidate_relatedness_profiles,
    ...
    paired_validations = _validate_paired_hashes(
        records, eligible_candidate_counts, candidate_relatedness_profiles
    )
    paired_hashes_path = output / "paired_invariance_hashes.json"
    ...
    _atomic_json(paired_hashes_path, {
        "paired_invariance_validated": True,
        "eligible_candidate_counts": list(eligible_candidate_counts),
        "candidate_relatedness_profiles": list(
            candidate_relatedness_profiles
        ),
        "groups": paired_validations,
    })
    elapsed = time.perf_counter() - started
    summary = {
        "schema_version": SCHEMA_VERSION,
        ...
        "effective_markers": list(effective_markers),
        "cohort_units": (
            len(replicate_ids) * len(scenarios)
            * len(eligible_candidate_counts)
            * len(candidate_relatedness_profiles)
            * len(effective_markers)
        ),
        ...
        "paired_invariance_validated": True,
        "paired_invariance_groups": len(paired_validations),
