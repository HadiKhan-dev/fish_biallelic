#%%
"""
pipeline_real.py — Real Data Pipeline (no simulation)

Load 200-SNP blocks from VCF → HDBSCAN discovery → L1-L4 assembly →
Tolerance painting → Pedigree inference → Phase correction.

Fully independent — no simulation pipeline dependencies.
Checkpoints in .pipeline_checkpoints_real/
"""
...
if __name__ == '__main__':
    ...
    def stage_complete(stage):
        return os.path.exists(os.path.join(CHECKPOINT_DIR, stage, '_done'))
    ...
    def contig_done(stage, r_name):
        return os.path.exists(os.path.join(CHECKPOINT_DIR, stage, f'{r_name}_done'))

    def save_contig(stage, r_name, data_dict):
        d = os.path.join(CHECKPOINT_DIR, stage)
        os.makedirs(d, exist_ok=True)
        path = os.path.join(d, f'{r_name}.pkl')
        with open(path, 'wb') as f:
            pickle.dump(data_dict, f, protocol=pickle.HIGHEST_PROTOCOL)
        sz = os.path.getsize(path) / (1024*1024)
        print(f"    [Checkpoint] {stage}/{r_name} ({sz:.1f} MB)")
        open(os.path.join(d, f'{r_name}_done'), 'w').close()
    ...
    def save_global(stage, data_dict):
        d = os.path.join(CHECKPOINT_DIR, stage)
        os.makedirs(d, exist_ok=True)
        path = os.path.join(d, '_global.pkl')
        with open(path, 'wb') as f:
            pickle.dump(data_dict, f, protocol=pickle.HIGHEST_PROTOCOL)
        sz = os.path.getsize(path) / (1024*1024)
    ...
    def load_global(stage):
        path = os.path.join(CHECKPOINT_DIR, stage, '_global.pkl')
        with open(path, 'rb') as f:
            return pickle.load(f)
...
if __name__ == '__main__':
    # =========================================================================
    # STAGE R1: VCF Loading + Block Discovery + Global Probabilities
    # =========================================================================
    ...
    if stage_complete(STAGE_R1):
        ...
        print(f"\n{'='*60}")
        print("VCF Loading + Block Haplotype Discovery")
        print(f"{'='*60}")
        ...
        for r_name in region_keys:
            ...
            # 1. Load VCF in 200-SNP non-overlapping blocks
            t0 = time.time()
            ...
            # 2. Concatenate all block reads → global arrays
            all_positions = []
            all_reads = []
            ...
            # Remove duplicate sites (from any edge overlap)
            ...
            # 3. Compute genotype probabilities
            ...
            # 4. HDBSCAN block haplotype discovery
            t0 = time.time()
            ...
            # 5. Save (global_probs, global_sites, block_results, avg_depth)
...
if __name__ == '__main__':
    # =========================================================================
    # STAGE R3: Refinement (if avg depth < 10x)
    # =========================================================================
    STAGE_R3 = "R03_refinement"
    ...
    if stage_complete(STAGE_R3):
        ...
        for r_name in region_keys:
            ...
            if avg_depth < REFINEMENT_DEPTH_THRESHOLD:
                ...
                save_contig(STAGE_R3, r_name, {'block_results': l2_refined_dd})
            else:
                print(f"  Depth >= {REFINEMENT_DEPTH_THRESHOLD}x → Skipping refinement")
                save_contig(STAGE_R3, r_name, {'block_results': block_results})

            del block_results, global_probs
            gc.collect()

        print(f"\nRefinement stage complete.")
        mark_stage_complete(STAGE_R3)

#%%
if __name__ == '__main__':
    # =========================================================================
    # STAGE R4: L1 Assembly
    # =========================================================================
    ...
    if stage_complete(STAGE_R4):
        ...
        for r_name in region_keys:
            ...
            super_blocks = hierarchical_assembly.run_hierarchical_step(
                ...
                batch_size=10, use_hmm_linking=False,
                beam_width=200, max_founders=12,
                max_sites_for_linking=2000, cc_scale=0.2,
        ...
        print(f"\nL1 assembly complete in {time.time()-start_time:.1f}s")
        mark_stage_complete(STAGE_R4)

#%%
if __name__ == '__main__':
    # =========================================================================
    # STAGE R5: L2 Assembly
    # =========================================================================
    ...
    if stage_complete(STAGE_R5):
        ...
        print(f"\nL2 assembly complete in {time.time()-start_time:.1f}s")
        mark_stage_complete(STAGE_R5)

#%%
if __name__ == '__main__':
    # =========================================================================
    # STAGE R6: L3 Assembly
    # =========================================================================
    ...
    if stage_complete(STAGE_R6):
        ...
        print(f"\nL3 assembly complete in {time.time()-start_time:.1f}s")
        mark_stage_complete(STAGE_R6)

#%%
if __name__ == '__main__':
    # =========================================================================
    # STAGE R7: L4 Assembly
    # =========================================================================
    ...
    if stage_complete(STAGE_R7):
        ...
        print(f"\nL4 assembly complete in {time.time()-start_time:.1f}s")
        mark_stage_complete(STAGE_R7)

#%%
if __name__ == '__main__':
    # =========================================================================
    # STAGE R8: Tolerance Painting
    # =========================================================================
    STAGE_R8 = "R08_tolerance_painting"

    if stage_complete(STAGE_R8):
        print(f"\n[RESUME] Skipping tolerance painting (checkpoint found)")
    else:
        print(f"\n{'='*60}")
        print("Tolerance Painting (Real Data)")
        print(f"{'='*60}")
        start_time = time.time()
        ...
        with paint_samples.PaintingPoolManager(num_processes=n_processes) as painter:
            for r_name in region_keys:
                ...
                tol_painting_result = painter.paint_chromosome(
                    ...
                    recomb_rate=5e-8, switch_penalty=10.0,
                    absolute_margin=5.0, batch_size=1
                )

                save_contig(STAGE_R8, r_name, {'tolerance_result': tol_painting_result})
                del discovered_block, global_probs, tol_painting_result
                gc.collect()

        print(f"\nTolerance painting complete in {time.time()-start_time:.1f}s")
        mark_stage_complete(STAGE_R8)

#%%
if __name__ == '__main__':
    # =========================================================================
    # STAGE R9: Pedigree Inference
    # =========================================================================
    ...
    if stage_complete(STAGE_R9):
        ...
        print(f"\n{'='*60}")
        print("Multi-Contig Pedigree Inference (Real Data)")
        print(f"{'='*60}")
...
if __name__ == '__main__':
    ...
    if stage_complete(STAGE_R10):
        ...
        print("\n" + "="*60)
        print("RUNNING: Phase Correction (Real Data)")
        print("="*60)
