...
def get_updated_transition_probabilities_unified(
    ...
    def _run_batched_pass(indices, is_forward):
        ...
        for i in indices:
            ...
            # Build hom->hom correction for the partner prior.
            # When u_out == u_in AND v_out == v_in, both chromosomes carry the
            # same haplotype at source and destination.  There is no phase 
            # ambiguity — both must take the same edge.  The partner prior
            # T(u_in -> v_in) is redundant because it's the SAME event as the
            # main transition, not independent evidence.
            # 
            # We precompute T_partner_corrected once outside the batch loop:
            # it replaces T_partner with 0.0 at hom->hom entries.
            T_partner_broad = T_matrix[np.newaxis, np.newaxis, :, np.newaxis, :]
            hom_hom_mask = np.zeros((1, n_curr, n_curr, n_next, n_next), dtype=bool)
            for a in range(n_curr):
                for b in range(n_next):
