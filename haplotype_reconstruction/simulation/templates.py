"""Founder sequence templates and reconstruction-to-truth matching."""
...
def _strip_block(block):
    ...
    """
    return core_haplotypes.BlockResult(
...
def _create_shm_array(arr, label=""):
    ...
    return core_parallel.create_shared_array(arr, name_key='shm_name', dtype_as_string=True)
...
def _worker_match_overlap_direct(args):
    ...
    for first_name in cur_ends.keys():
        ...
        for second_name in next_ends.keys():
            ...
            if common_size > 0:
                haps_dist = 100 * core_numerics.calc_distance(
...
def _worker_combined_best_hap_matches_direct(args):
    ...
    return simulation_metrics.combined_best_hap_matches(block)
...
def _worker_hap_matching_comparison_direct(args):
    ...
    return simulation_metrics.hap_matching_comparison(
...
def _worker_combine_chained_blocks_direct(args):
    ...
    for i in range(len(light_blocks)):
        ...
        full_blocks.append(core_haplotypes.BlockResult(
    ...
    reconstituted = core_haplotypes.BlockResults(full_blocks)
...
def _worker_get_similarities_direct(args):
    ...
    block_idx, block = args
    return simulation_metrics.get_block_hap_similarities(block)
...
def match_haplotypes_by_samples_probabalistic(full_haps_data, num_processes=16):
    ...
    return [
        (block_haps_names, forward_match_scores),
        (block_haps_names, backward_match_scores),
        (block_haps_names, combined_scores)
    ]
...
def generate_chained_block_haplotypes(haplotype_data, nodes_list, combined_scores, num_haplotypes,
    ...
    if similarity_matrices is None:
        ...
        similarity_matrices = [
            simulation_metrics.get_block_hap_similarities(block) for block in haplotype_data
        ]
    ...
    for ite in range(num_haplotypes):
        ...
        found_hap = scorings_to_optimal_path(
            best_scores,
            nodes_copy,
            current_node_scores,
            current_edge_scores
        ...
        if i < num_blocks - 1:
            ...
            if overlap_len_next > 0:
                ...
                for k in range(process_len):
                    new_val = core_numerics.combine_probabilities(
...
def combine_all_blocks_to_long_haps(all_haps,
    ...
    try:
        ...
        with core_parallel.safe_forkserver_pool(min(num_processes, len(task_args)),
        ...
        core_parallel.close_shared_memory(shm_handles, unlink=True)
...
def build_founder_templates(block_results, num_long_haps,
    ...
    for b in block_results:
        ...
        if len(b.positions) == 0:
            continue
        if len(b.haplotypes) == 0:
            continue
        if b.keep_flags is not None and np.sum(b.keep_flags) == 0:
            continue
    ...
    hap_matching_overlap = match_haplotypes_by_overlap_probabalistic(
        valid_blocks,
        num_processes=num_processes
    )

    # Run Shared Memory Optimized Sample Matching
    hap_matching_samples = match_haplotypes_by_samples_probabalistic(
        valid_blocks,
        num_processes=num_processes
    )
    ...
    with core_parallel.safe_forkserver_pool(num_processes) as p:
    ...
    final_long_haps = combine_all_blocks_to_long_haps(
        valid_blocks,
        chained_block_haps,
        num_processes=num_processes
    )
...
import haplotype_reconstruction.simulation.metrics as simulation_metrics
