...
class BatchedSearchConfig:
    ...
    def __post_init__(self):
        for value in (self.max_rounds,self.fits_per_round,self.birth_starts,self.max_lookahead_rounds,self.cut_starts,self.cut_sweeps,self.residual_samples):
...
def _improve_cut(weights,initial,order,sweeps):
    ...
    for _ in range(sweeps):
        ...
        for i in order:
            ...
            if delta>0:side[i]=not side[i];changed=True
...
def bounded_cuts(weights,config,max_ties):
    ...
    return tuple(sorted(seen.values(),key=lambda side:(-modes._cut_score(weights,side)[0],side.tobytes()))[:max_ties])
...
def residual_rows(mode,evidence,reads,settings,growth,residual_workspace):
    ...
    for row in rows:
        ...
        if key not in existing:unique.setdefault(key,np.asarray(row,dtype=np.int64))
...
def run(evidence,reads,seeds,candidate_rows,settings,workspace,growth,residual_workspace):
    ...
    active=growth.active_sample_mask;n_sites=evidence.shape[1]
    ...
    archive={};cache={};scores={};fitted_count=0;steps=[];exhausted=False
    def fit(starts):
        ...
        for start in starts:
            ...
            if np.any((panel!=0)&(panel!=1)):raise ValueError('panel seeds must be binary')
        ...
        for mode in search._fit_panel_starts(evidence,retained,fit_config,workspace):
            ...
            if previous is None or search._mode_order(mode)<search._mode_order(previous):archive[mode.canonical_key]=mode
        grouped={}
        for mode in archive.values():
            old=grouped.get(mode.k)
            if old is None or search._mode_order(mode)<search._mode_order(old):grouped[mode.k]=mode
        ...
        for k,mode in grouped.items():
            if mode.canonical_key in cache:scores[k]=cache[mode.canonical_key]
    ...
    exploration_parent=None;expanded=set();non_improving_rounds=0
    for iteration in range(cfg.max_rounds):
        ...
        k=parent.k;expanded.add(parent.canonical_key)
        ...
        if k>1:
            for count in dict.fromkeys((int(np.sum(occupancy==0)),max(1,k//4),max(1,k//2),1)):
                if 0<count<k:starts.append(np.delete(parent.haplotypes,order[:count],axis=0))
        if rows:
            count=min(k,len(rows));replacement=parent.haplotypes.copy()
            replacement[order[:count]]=rows[:count];starts.append(replacement)
        ...
        before=len(cache);before_fits=fitted_count;fit(starts)
        ...
        if new.log_score<=selected.log_score+settings.score_tolerance:
            ...
            if non_improving_rounds>=cfg.max_lookahead_rounds:
                stop='no_improving_batched_lookahead';break
            ...
            if alternatives:
                exploration_parent=min(alternatives,key=lambda mode:(-mode.k,mode.total_nll,mode.canonical_key))
            else:
                stop='no_improving_batched_neighbour';break
        else:
            non_improving_rounds=0
        if exhausted:stop='exact_score_budget';break
    ordered=sorted(scores.values(),key=search._score_sort_key);selected=ordered[0]
    limits=('batched_candidate_neighbourhood',)+(('exact_score_budget',) if exhausted else ())
    if any(item.mean_field_diagnostic.n_mean_field_not_converged for item in ordered):limits+=('mean_field_nonconvergence',)
    ...
    logs=np.asarray([item.log_score for item in ordered]);weights=np.exp(logs-logs.max());weights/=weights.sum()
    grouped={}
    for mode in archive.values():grouped.setdefault(mode.k,[]).append(mode)
