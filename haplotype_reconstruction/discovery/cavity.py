...
_CAVITY_PREDICTION_TILE_BYTES = 8 * 1024 * 1024
...
def _score_mode_cavity_kernel(
    ...
    Exact equal assignment/evidence cases share per-site posterior solves.
    Each sample still accumulates its own predictions and diagnostics in the
    original site/founder order, followed by sample-ordered reduction.
    """
    ...
    emissions = np.zeros((n_samples, n_states), dtype=np.float64)
    bytes_per_site = 8 * max(1, n_samples) * (n_states + 2 * k + 6)
    tile_width = max(1, _CAVITY_PREDICTION_TILE_BYTES // bytes_per_site)
    for site_start in range(0, n_sites, tile_width):
        site_stop = min(n_sites, site_start + tile_width)
        (representatives, predictions, entropy_terms,
         diagnostics, spreads) = grouped_cavity_predictions(
        ...
        for heldout in prange(n_samples):
            heldout_iterations = iterations_by_heldout[heldout]
            heldout_not_converged = not_converged_by_heldout[heldout]
            heldout_zero_support = zero_support_by_heldout[heldout]
            heldout_entropy = entropy_by_heldout[heldout]
            heldout_alternate_wins = alternate_wins_by_heldout[heldout]
            heldout_initialization_spread = (
                initialization_spread_by_heldout[heldout]
            )
            # Continue each sample's original site/founder accumulation across
            # tiles. A group is computation reuse, not a change in its weight.
            for offset in range(site_stop - site_start):
                representative = representatives[offset, heldout]
                heldout_iterations += diagnostics[offset, representative, 0]
                heldout_not_converged += diagnostics[offset, representative, 1]
                heldout_zero_support += diagnostics[offset, representative, 2]
                heldout_alternate_wins += diagnostics[offset, representative, 3]
                heldout_initialization_spread += spreads[offset, representative]
                for term in range(2 * k):
                    heldout_entropy -= entropy_terms[offset, representative, term]
                for state in range(n_states):
                    emissions[heldout, state] += predictions[
                        offset, representative, state
                    ]
    ...
    for heldout in prange(n_samples):
        ...
        held_state = state_lookup[assignments[heldout, 0], assignments[heldout, 1]]
        for state in range(n_states):
            log_state_predictive[state] = full_log_state_mass[state]
        log_state_predictive[held_state] = deleted_log_state_mass[held_state]
        for state in range(n_states):
            log_state_predictive[state] += emissions[heldout, state]
