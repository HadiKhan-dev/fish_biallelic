...
def compute_cc(batch_blocks, num_samples, cc_scale=0.5):
    ...
    return discovery_objectives.compute_founder_complexity_cost(
        cc_scale, num_samples, avg_snps, n_blocks=len(batch_blocks)
    )


def compute_subblock_emissions(input_blocks, global_probs, global_sites,
                               snps_per_bin, num_threads=None):
    ...
    for block in input_blocks:
        ...
        block_samples = assembly_chimera_kernels._gather_samples_numba(global_probs, indices)
        ...
        if panel.keys != tuple(hap_keys):
            raise ValueError("founder panel keys disagree with haplotype keys")
        block_keep = getattr(block, "keep_flags", None)
    ...
    for b_idx, em_data in enumerate(sub_emissions):
        ...
        if n_pairs > 0:
            if parallel_build:
                ...
                assembly_chimera_kernels._build_tensor_block_numba(
                    tensor, bin_em, local_indices,
                    s0, s1, K, n_bins_b, bin_offset,
                )
        bin_offset += n_bins_b
...
def score_path_set(path_set, sub_emissions, penalty, num_samples,
    ...
    if num_threads is not None:
        assembly_chimera_kernels._resolve_threads(num_threads)
    return float(np.sum(discovery_objectives.viterbi_score_selection(tensor, float(penalty))))
...
def score_path_sets_parallel(path_sets, sub_emissions, penalty, num_samples,
    ...
    for K, group_items in groups.items():
        ...
        for chunk_start in range(0, len(group_items), cs):
            ...
            scores = assembly_chimera_kernels._batched_viterbi_score(stacked, float(penalty))
            del stacked
            for j in range(n_chunk):
...
def beam_warmstart_select(beam_results, fast_mesh, sub_emissions,
    ...
    for beam_idx, kp in zip(unique_beam_idx, unique_keypaths):
        ...
        bic_cand = discovery_objectives.compute_outer_bic_from_log_likelihood(
        ...
        if bic_cand < bic_S:
            ...
            while len(selected_keypaths) > 1:
                ...
                lls = score_path_sets_parallel(
                    ...
                    num_threads=num_threads)
                bics = [
                    discovery_objectives.compute_outer_bic_from_log_likelihood(
                        K_cur - 1, ll, batch_cc
                    ) for ll in lls
                ]
...
def paint_samples_viterbi(path_set, sub_emissions, penalty, num_samples,
    ...
    if num_threads is not None:
        assembly_chimera_kernels._resolve_threads(num_threads)
    return assembly_chimera_kernels._viterbi_traceback(tensor, float(penalty)), K
