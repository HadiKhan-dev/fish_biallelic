...
def select_and_resolve(beam_results, fast_mesh, batch_blocks,
    ...
    Args:
        ...
        Founder emissions use authoritative complete calls and one shared
            latent allele for diagonal founder-pair states.
    ...
    sub_em = assembly_chimera_scoring.compute_subblock_emissions(
    ...
    def build_tensor_sel(subset_indices):
        ...
        for b_i, em_data in enumerate(sub_em):
            ...
            if n_pairs > 0:
                assembly_chimera_kernels._build_tensor_block_numba_par(
    ...
    def build_tensors_threaded(subset_list):
        nt = assembly_chimera_kernels._resolve_threads(num_threads)
    ...
    def score_subset(subset_indices):
        ...
        assembly_chimera_kernels._resolve_threads(num_threads)
        return float(np.sum(
            discovery_objectives.viterbi_score_selection(tensor, float(pen_sel))))
    ...
    # =========================================================================
    _warmstart_selected, _warmstart_bic = assembly_chimera_scoring.beam_warmstart_select(
    ...
    for k in range(step1_max_iters):
        ...
        cand_idx_for_p_s1 = assembly_chimera_kernels._build_cand_idx_for_p(stride_s1, cand_pos_s1)
        ...
        for cs in range(0, len(remaining), max_chunk):
            ...
            for _s0 in range(0, num_samples, _sc_eff):
                ...
                assembly_chimera_kernels._build_cand_sv_numba(
                ...
                partial = assembly_chimera_kernels._batched_viterbi_score_split(
    ...
    for _outer_iter in range(MAX_OUTER_ITERATIONS):
        ...
        def precompute_base_max(base_set):
            ...
            for b_i, em_data in enumerate(sub_em):
                ...
                base_maxes.append(assembly_chimera_kernels._precompute_base_max_block_numba(
                    bin_em,
        ...
        def cheap_score_all(base_maxes, temp_set, candidates):
            ...
            for b_i, em_data in enumerate(sub_em):
                ...
                # Bit-exact with fastmath=False — see kernel docstring.
                hap_contribs = assembly_chimera_kernels._cheap_score_all_block_numba(
                    bin_em,
        ...
        def _run_1for1_round(selected, get_candidates_fn, cur_score):
            ...
            cand_idx_for_p = assembly_chimera_kernels._build_cand_idx_for_p(K, cand_pos)
            ...
            for i in range(K):
                ...
                tmpl_full = np.empty((num_samples, total_bins, n_pairs),
                                     dtype=np.float32)
                assembly_chimera_kernels._build_template_numba(tmpl_full, stacked_bin_em,
        ...
        def _run_2for1_round(selected, cur_score):
            ...
            current_bic = discovery_objectives.compute_outer_bic_from_log_likelihood(K, cur_score, batch_cc)
            K_result = K - 1
            ...
            # at K=6 2for1).
            cand_idx_for_p = assembly_chimera_kernels._build_cand_idx_for_p(K_result, cand_pos)
            n_cand_pairs = 2 * cand_pos + 1
        ...
        while len(selected) > 1:
            cur_ll = score_subset(selected)
            k_now = len(selected)
            cur_bic = discovery_objectives.compute_outer_bic_from_log_likelihood(k_now, cur_ll, batch_cc)
        ...
        for iteration in range(max_cr_iterations):
            # 4a. Paint samples
            sp, K_cr = assembly_chimera_scoring.paint_samples_viterbi(
                current_paths, sub_em, paint_penalty, num_samples,
                num_threads=num_threads)

            # 4b. Find hotspots
            hotspots = assembly_chimera_scoring.find_hotspots(
            ...
            current_ll = assembly_chimera_scoring.score_path_set(
            ...
            all_scores = assembly_chimera_scoring.score_path_sets_parallel(
            ...
            while len(current_paths) > 1:
                cur_ll = assembly_chimera_scoring.score_path_set(
                    current_paths, sub_em, pen_sel, num_samples,
                    num_threads=num_threads)
    ...
    if len(current_paths) < 2:
        ...
        for _step5_iter in range(step5_max_iters):
            ...
            _step5_sample_paths, _step5_K_check = assembly_chimera_scoring.paint_samples_viterbi(
                current_paths, sub_em, pen_sel, num_samples,
                num_threads=num_threads)
            ...
            for _step5_sb in range(1, len(_step5_bin_offsets)):
                ...
                if _step5_b_bin == 0 or \
                    ...
                    continue
                _step5_W = assembly_chimera_scoring._step5_build_W_at_boundary(
                ...
                _step5_sigma_v10, _step5_active = assembly_chimera_scoring._step5_hungarian_active(
                ...
                # per boundary.
                _step5_v11_candidates = assembly_chimera_scoring._step5_murty_active(
...
import haplotype_reconstruction.assembly.chimera_scoring as assembly_chimera_scoring
