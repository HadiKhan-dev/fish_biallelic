    ...
    for task in prange(len(jobs)):
        ...
        for variant in range(2):
            ...
            for site in range(left, right):
                if runs[sample, site] < 0:
                    continue
                if first < 0:
                    first = site
                else:
                    score += _boundary(variant, variant, previous, site, positions, runs[sample], rate, mean)
                previous = site
...
def _query(tree, base, node, lo, hi, left, right, carry, positions, runs, rate, mean):
    ...
    while top:
        ...
        if right <= a or b <= left:
            value = _value(tree, absolute, inherited)
        elif left <= a and b <= right:
            value = _value(tree, absolute, inherited ^ 1)
        else:
            middle = (a + b) // 2
...
def _update(tree, base, node, lo, hi, left, right, positions, runs, rate, mean):
    ...
    while top:
        ...
        if left <= a and b <= right:
            ...
            continue
        if tree[4][absolute]:
            _flip(tree, base + 2 * local)
            _flip(tree, base + 2 * local + 1)
            tree[4][absolute] = 0
        middle = (a + b) // 2
