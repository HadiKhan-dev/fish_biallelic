...
def write_chromosome_outputs(result, output):
    contig = result["contig"]; curve = result["map"]
    ...
    events=[];coverage=[];called_coverage=[];meioses=[]
    for edge,(parent,child,slot) in enumerate(zip(result["edge_parent"],result["edge_child"],result["edge_child_slot"])):
        labels={"parent":result["sample_ids"][parent],"child":result["sample_ids"][child],"parent_slot":int(slot)}
        ...
        for left,right,component in result["informative_spans"][edge]:
            coverage.append({**labels,"left_bp":int(left),"right_bp":int(right),"component_id":int(component)})
        for left,right,component in result["callable_spans"][edge]:
            called_coverage.append({**labels,"left_bp":int(left),"right_bp":int(right),"component_id":int(component)})
    ...
    axes[1].set_ylabel("Effective meioses");axes[1].set_xlabel("Position (Mb)")
    axes[0].set_title(f"{contig}: missing-aware recombination and exposure")
    figure.tight_layout()
    path=output/f"{contig}.png";temporary=output/f".{contig}.png.tmp"
    figure.savefig(temporary,format="png",dpi=150);plt.close(figure);temporary.replace(path)
...
def run_recombination(checkpoint_store,contigs,sample_ids,*,pedigree_payload,output_dir,
    ...
    for contig in contigs:
        ...
        stat=path.stat();files.append({"path":str(path),"size":stat.st_size,"mtime_ns":stat.st_mtime_ns})
    ...
    output=Path(output_dir)/"recombination_map";output.mkdir(parents=True,exist_ok=True)
    if checkpoint_store.stage_complete(RECOMBINATION_STAGE) and checkpoint_store.global_done(RECOMBINATION_STAGE):
        ...
        if saved["identity"]!=identity:raise ValueError("Stage12 completed summary identity differs")
    ...
    with core_parallel.numba_thread_scope(workers):
        for contig in contigs:
            if checkpoint_store.contig_done(RECOMBINATION_STAGE,contig):
                ...
                if result["identity"]!=identity:raise ValueError("Stage12 chromosome identity differs")
            else:
                phase=checkpoint_store.load_contig(refinement_pipeline.FINAL_PHASE_STAGE,contig,nthreads=workers)
