...
def _update(tree,base,node,lo,hi,left,right,toggle,positions,genetic,
    ...
    while top:
        ...
        if left<=a and b<=right:
            _flip_node(tree,absolute,toggle);continue
        if tree[4][absolute]:
            tag=int(tree[4][absolute])
            _flip_node(tree,base+2*local,tag);_flip_node(tree,base+2*local+1,tag)
            tree[4][absolute]=0
        middle=(a+b)//2
...
class EdgeIntervalProducts:
    ...
    def __init__(self,data,candidates,adjacent,config,maximum_leaf_markers=4096):
        ...
        stationary = config.phase_artifact_rate / (config.phase_artifact_rate + 1/config.phase_artifact_mean_bp)
