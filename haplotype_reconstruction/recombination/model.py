...
"""
    ...
    for i in range(n):
        if i and not resets[i]:
            ...
            if genetic_positions_morgans is None:
                ...
                theta = -.5 * math.expm1(
                    -2 * (genetic_positions_morgans[i] - genetic_positions_morgans[i - 1])
                )
            changed = -math.expm1(-(artifact_rate + exit_rate) * distance)
            enter = stationary * changed
            leave = (1 - stationary) * changed
            for a in range(4):
                for b in range(4):
                    p = theta if (a >> 1) != (b >> 1) else 1 - theta
                    if a & 1:
                        p *= leave if not b & 1 else 1 - leave
                    else:
                        p *= enter if b & 1 else 1 - enter
                    transition[a, b] = p
        total = 0.
        for b in range(4):
            ...
            if i and not resets[i]:
                value = 0.
                for a in range(4):
                    value += forward[i - 1, a] * transition[a, b]
    ...
    clean = np.empty(n)
    back = np.ones(4)
    following = np.empty(4)
    switch = np.zeros(n, dtype=np.float64)
    for i in range(n - 1, -1, -1):
        w0 = forward[i, 0] * back[0]
        w1 = forward[i, 1] * back[1]
        w2 = forward[i, 2] * back[2]
        w3 = forward[i, 3] * back[3]
        weight = w0 + w1 + w2 + w3
        origin[i] = (w0 + w1) / weight
        clean[i] = (w0 + w2) / weight
        if i and not resets[i]:
            distance = positions[i] - positions[i - 1]
            if genetic_positions_morgans is None:
                theta = -.5 * math.expm1(-2 * rate * distance)
...
def _one_meiosis(calls, displayed_phase, internal_phase, positions, components,
    ...
    for i in range(len(events)):
        for k in range(5):
            event_array[i, k] = events[i][k]
    for i in range(len(spans)):
        for k in range(3):
            span_array[i, k] = spans[i][k]
...
def edge_observations(calls, orientation, positions, runs, parent, child, slot,
    ...
    for j in range(len(positions)):
        ...
        if flip_left <= j < flip_right:
            if parent == flip_sample:
                p_phase ^= 1
            if child == flip_sample:
                c_phase ^= 1
...
def _decode_meioses(calls, displayed_phase, internal_phase, positions, components,
    ...
    soft_counts = np.zeros((n, len(bins) - 1))
    soft_exposure = np.zeros_like(soft_counts)
...
"""
    ...
    if soft_counts is not None:
        expected = soft_counts.sum(axis=0)
        exposure = soft_exposure.sum(axis=0)
    ...
    for left, right in union:
        _add_interval(bins, covered, left, right, 1.)
...
"""
    ...
    if chromosome_map is not None:
        kwargs["chromosome_map"] = chromosome_map
