...
class FamilyRefinementConfig:
    ...
    def validated(self):
        ...
        for name in (
            ...
            "checkpoint_every",
            "gauge_move_interval",
            "branch_cluster_after",
            "branch_cluster_interval"
        ):
...
def grouped_edges(parents, children, slots, samples):
    ...
    for edge, (parent, child, slot) in enumerate(zip(parents, children, slots)):
        group = node_group[child]
        group_parent[group, slot] = parent
        group_edge[group, slot] = edge
...
def prepare_family_workspace(genotype_likelihoods, observed, reference_alleles, positions,
    ...
    if cfg.scaffold_scope != "roots":
        raise ValueError("family refinement requires root-only scaffold phase anchors")
    gl = np.asarray(genotype_likelihoods, dtype=np.float64)
    obs = np.asarray(observed, dtype=bool)
    reference = np.asarray(reference_alleles, dtype=np.int8)
    pos = np.asarray(positions, dtype=np.int64)
    bins = np.asarray(phase_bins)
    components = np.asarray(component_ids)
    samples, sites = gl.shape[:2]
    if gl.shape != (samples, sites, 3) or sites == 0 or samples != len(sample_ids):
        raise ValueError("invalid GL axes")
    if obs.shape != (samples, sites) or reference.shape != (samples, sites, 2):
        raise ValueError("invalid scaffold axes")
    ...
    parent_counts = np.bincount(children, minlength=samples)
    roots = parent_counts == 0
    ...
    if np.any(invalid):
        raise ValueError("invalid allele evidence")
...
def refine_family(genotype_likelihoods, observed, reference_alleles, positions, phase_bins,
    ...
    belief = beliefs()
    converged = bool(messages.deltas and messages.deltas[-1] < cfg.tolerance)
    ...
    while messages.iteration < cfg.max_iterations and not converged:
        ...
        seg_delta = refinement_messages.chain_messages(
            emissions,
            messages,
            theta,
            damping,
        ...
        messages.deltas = (*messages.deltas, change)
        converged = change < cfg.tolerance
        belief = beliefs()
        gauge_changed = False
        ...
        if (not converged and not gauge_changed and messages.iteration >= cfg.branch_cluster_after
            ...
            if refinement_messages.add_unstable_clusters(
                messages,
                gp,
                ge,
                gc,
...
def _adjacency(samples, parents, children):
    ...
    for edge, (parent, child) in enumerate(zip(parents, children)):
        neighbours[int(parent)].append(edge)
        neighbours[int(child)].append(edge)
