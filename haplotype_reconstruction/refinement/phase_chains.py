...
def compact_phase(emissions, starts, trans, anchor, paths):
    bins = len(starts) - 1
    compact = np.empty((bins, 2))
    anchor_bin = -1
    for block in range(bins):
        a = 0.
        b = 0.
        for site in range(starts[block], starts[block + 1]):
            a += math.log(emissions[site, 0])
            b += math.log(emissions[site, 1])
        if starts[block] <= anchor < starts[block + 1]:
            ...
            b = -np.inf
        maximum = max(a, b)
        compact[block, 0] = math.exp(a - maximum)
        compact[block, 1] = math.exp(b - maximum)
...
def emission_pair(belief_row, orientation_value, phase_value, phase_error):
    ...
    f1 = p if orient == 0 else 1 - p
    f2 = 1 - p if orient == 0 else p
...
def phase_update(belief_log, orientation, phase, phase_theta, anchors, phase_error, damping, write_messages):
    ...
    for index in prange(len(active_samples)):
        ...
        posterior, switches, path = compact_phase(
            emissions,
            starts,
            trans,
            anchors[sample],
        ...
        for block in range(len(starts) - 1):
            ...
            for site in range(starts[block], starts[block + 1]):
                if write_messages:
                    c0 = p0 / emissions[site, 0]
                    c1 = p1 / emissions[site, 1]
                    old = phase[sample, site]
                    new = (1 - damping) * old + damping * c0 / (c0 + c1)
                    delta[sample] = max(delta[sample], abs(new - old))
                    phase[sample, site] = new
                else:
                    probability[sample, site] = p0
                    paths[sample, site] = path[block]
                    if site + 1 < sites:
                        links[sample, site] = 1.
...
def phase_context_view(belief, reference, orientation, phase, phase_theta, anchors, parent_counts,
    ...
    starts, trans = phase_segments(phase_theta, sites)
    high = np.float32(phase_threshold)
    low = np.float32(1 - phase_threshold)
    for sample in prange(samples):
        has_frame = False
        for site in range(sites):
        ...
        for block in range(len(starts) - 1):
            ...
            for site in range(starts[block], starts[block + 1]):
                inferred[sample, site] = inferred_flip
                emitted[sample, site] = displayed_flip
                a = reference[sample, site, displayed_flip]
                b = reference[sample, site, 1 - displayed_flip]
                # Called scaffold alleles are unchanged. Only missing sites
                # require genotype probabilities for conditional context.
                if a < 0 or b < 0:
                    q0 = np.exp(belief[sample, site, 0])
                    q1 = np.exp(belief[sample, site, 1])
                    q2 = np.exp(belief[sample, site, 2])
                    q3 = np.exp(belief[sample, site, 3])
                    if inferred_flip != displayed_flip:
                        q1, q2 = q2, q1
                    if (a >= 0) != (b >= 0):
                        ...
                        if not phase_supported and has_frame:
                            first = second = (first + second) / 2.
                        if max(first, 1 - first) >= .98:
                            a = int(first >= .5)
                        if max(second, 1 - second) >= .98:
                            b = int(second >= .5)
