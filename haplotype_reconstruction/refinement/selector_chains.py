...
def paired_chain_messages(emission, theta, diagnostics=True):
    ...
    for site in range(n - 2, -1, -1):
        ...
        total = sum(values)
        for state in range(4):
            beta[site, state] = values[state] / total
    ...
    for site in range(n):
        ...
        for state in range(4):
            ...
            if diagnostics:
                posterior[site, state] /= post_total
        if diagnostics and site + 1 < n:
            ...
            switches[site, 0] = jump0 / denominator
            switches[site, 1] = jump1 / denominator
...
def count_active(fixed, gp):
    ...
    for group in prange(len(gp)):
        ...
        for site in range(fixed.shape[1]):
            if (p0 >= 0 and not fixed[p0, site]) or (p1 >= 0 and not fixed[p1, site]):
                counts[group] += 1
    return counts
...
def fill_support(fixed, gp, offsets, log_correlation, resets):
    ...
    theta = np.zeros(offsets[-1])
    for group in prange(len(gp)):
        p0, p1 = gp[group]
        cursor = offsets[group]
        previous = -1
        for site in range(fixed.shape[1]):
            if not ((p0 >= 0 and not fixed[p0, site]) or (p1 >= 0 and not fixed[p1, site])):
                continue
            indices[cursor] = site
            if previous >= 0:
...
def prepare_selector_support(base, gp, theta):
    fixed = fixed_nodes(base)
    counts = count_active(fixed, gp)
    offsets = np.r_[0, np.cumsum(counts)].astype(np.int64)
    resets = np.r_[0, np.cumsum(theta == .5)].astype(np.int64)
    logs = np.zeros(len(theta))
    np.log1p(-2 * theta, out=logs, where=theta < .5)
...
class ChainCache:
    def __init__(self, support, max_bytes=MAX_CACHE_BYTES):
        ...
        count = int(offsets[self.groups])
        lengths = np.diff(offsets[:self.groups + 1])
    ...
    def invalidate(self):
        self.arrays[6].fill(False)
        self.arrays[7].fill(False)
        for cache in self.branches.values():
            cache.invalidate()
...
def _physical_tile_mapping(offsets, indices, tiles):
    ...
    for group in prange(len(offsets) - 1):
        ...
        for tile in range(tiles):
            left = cursor
            while cursor < end and indices[cursor] < (tile + 1) * 32:
                cursor += 1
            if cursor > left:
    ...
    for job in prange(len(active_groups)):
        ...
        if group >= len(valid):
            ...
            for i in range(start, stop):
                for state in range(4):
                    local[i - start, state] = emissions[group, indices[i], state]
            ...
            for k in range(1, 5):
                stats[group, k] = length
            continue
        was_valid = valid[group]
        first_block = block_offsets[group]
        blocks = block_offsets[group + 1] - first_block
        dirty = np.zeros(blocks, dtype=np.bool_)
        forward = np.zeros(blocks, dtype=np.bool_)
        backward = np.zeros(blocks, dtype=np.bool_)
        ...
        if was_valid and producer_dirty is not None:
            ...
            for tile in range(producer_dirty.shape[1]):
                if producer_dirty[group, tile] and first[group, tile] >= 0:
                    for block in range(first[group, tile], stop_block[group, tile]):
                        inspect[block] = True
        for block in range(blocks):
            if not inspect[block]:
                continue
            lo = start + block * BLOCK_MARKERS
            hi = min(stop, lo + BLOCK_MARKERS)
            for i in range(lo, hi):
                different = not was_valid
                for state in range(4):
                    value = emissions[group, indices[i], state]
                    if was_valid and value != saved[i, state]:
                        different = True
                    saved[i, state] = value
                if different:
                    dirty[block] = True
        ...
        for block in range(blocks):
            ...
            for i in range(lo, hi):
                ...
                total = 0.
                for state in range(4):
                    predicted[i, state] = p[state]
                    alpha[i, state] = p[state] * saved[i, state]
                    total += alpha[i, state]
                for state in range(4):
                    alpha[i, state] /= total
        ...
        for block in range(blocks - 1, -1, -1):
            ...
            for i in range(hi - 1, lo - 1, -1):
                if i == stop - 1:
                    ...
                    total = sum(values)
                    for state in range(4):
                        beta[i, state] = values[state] / total
            for state in range(4):
                boundary[key, state] = saved[lo, state] * beta[lo, state]
...
def update_incremental_chains(emissions, current, damping, keep, support, cache, dirty_tiles=None):
    if cache.damping != damping:
        cache.arrays[7].fill(False)
        cache.damping = damping
...
def branch_cavity(emission, theta):
    ...
    cavity = np.empty((sites, 16))
    row = np.full(16, 1 / 16)
    following = np.ones(16)
    for site in range(sites):
        if site:
            mix16_inplace(row, theta[site - 1])
        total = 0.
        for state in range(16):
            predicted[site, state] = row[state]
            row[state] *= emission[site, state]
            total += row[state]
        for state in range(16):
            row[state] /= total
...
def update_branch(emissions, theta, saved, alpha, predicted, beta, cavity, boundary, valid, stats):
    ...
    backward = np.zeros(blocks, dtype=np.bool_)
    row = np.empty(16)
    previous = np.empty(16)
    for site in range(sites):
        different = not ready
        for state in range(16):
            value = emissions[site, state]
            if ready and value != saved[site, state]:
                different = True
            saved[site, state] = value
        if different:
            dirty[site // BLOCK] = True
            stats[0] += 1
    carry = not ready
    for block in range(blocks):
        lo = block * BLOCK
        hi = min(sites, lo + BLOCK)
        if not (dirty[block] or carry):
            continue
        if ready:
            for state in range(16):
                previous[state] = alpha[hi - 1, state]
        for state in range(16):
            row[state] = 1 / 16 if lo == 0 else alpha[lo - 1, state]
        for site in range(lo, hi):
            ...
            for state in range(16):
                predicted[site, state] = row[state]
                row[state] *= saved[site, state]
                total += row[state]
            for state in range(16):
                row[state] /= total
                alpha[site, state] = row[state]
        carry = not ready
        if ready:
            for state in range(16):
                if alpha[hi - 1, state] != previous[state]:
                    carry = True
        forward[block] = True
        stats[1] += hi - lo
    carry = not ready
    for block in range(blocks - 1, -1, -1):
        ...
        if ready:
            for state in range(16):
                previous[state] = boundary[block, state]
        if hi == sites:
            row[:] = 1.
        else:
            for state in range(16):
                row[state] = saved[hi, state] * beta[hi, state]
            mix16_inplace(row, theta[hi - 1])
            total = np.sum(row)
            for state in range(16):
                row[state] /= total
        for site in range(hi - 1, lo - 1, -1):
            for state in range(16):
                beta[site, state] = row[state]
            if site > lo:
                for state in range(16):
                    row[state] *= saved[site, state]
                mix16_inplace(row, theta[site - 1])
        ...
        for state in range(16):
            ...
            if ready and boundary[block, state] != previous[state]:
                carry = True
        backward[block] = True
        stats[2] += hi - lo
    for block in range(blocks):
        if not (forward[block] or backward[block]):
            continue
        lo = block * BLOCK
        hi = min(sites, lo + BLOCK)
        for site in range(lo, hi):
