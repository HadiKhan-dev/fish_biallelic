...
def root_mode_flips(belief, orientation, phase, parents, edge_group, edge_slot,
    ...
    flips = np.zeros((samples, sites), dtype=np.int8)
    ...
    for index in prange(len(active_roots)):
        ...
        for site in range(sites):
            ...
            orient = orientation[root, site]
            current = phase_error + (1 - 2 * phase_error) * phase[root, site]
            for state in range(4):
                if orient < 0 or state == 0 or state == 3:
                    factor = .5
                else:
                    factor = current if state == (1 if orient == 0 else 2) else 1 - current
        ...
        for row in range(len(outgoing)):
            ...
            shift = 1 - edge_slot[edge]
            for site in range(sites):
                child_paths[row, site] = (joint_maps[group, site] >> shift) & 1
...
def apply_joint_root_flips(flips, phase, parent_log, joint, parents, group_parent):
    ...
    for sample in prange(samples):
        for site in range(sites):
            if flips[sample, site]:
                phase[sample, site] = 1 - phase[sample, site]
    ...
    for group in prange(len(group_parent)):
        ...
        for site in range(sites):
            ...
            if permutation:
                ...
                for state in range(4):
                    joint[group, site, state] = previous[state ^ permutation]
    ...
    for site in range(n):
        ...
        for state in range(4):
            ...
            for previous in range(4):
                bits = previous ^ state
                changes = (bits >> 1) + (bits & 1)
                if changes and t == 0:
                    continue
                value = score[previous] + (2 - changes) * math.log(1 - t)
                if changes:
                    value += changes * math.log(t)
                if value > best:
                    best = value
                    arg = previous
            new[state] = best + math.log(max(1e-300, emission[site, state]))
            back[site, state] = arg
        score = new
    result = np.empty(n, dtype=np.int8)
    state = np.argmax(score)
