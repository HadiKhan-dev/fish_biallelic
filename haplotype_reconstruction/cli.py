...
def main(argv=None):
    parser=argparse.ArgumentParser(description='Missing-aware haplotype reconstruction for experimental crosses.')
    ...
    for name,help_text in (
        ...
        command.add_argument('--config',type=Path,help='TOML configuration; paths are relative to the current directory.')
        ...
        if name=='simulate':
            ...
            command.add_argument('--generations',type=int,nargs='+',help='Individuals in successive cohorts, default20 100 200.')
            command.add_argument('--generating-map',help='Optional simulation-generating map, independent of inference.')
            command.add_argument('--generating-rate-cm-per-mb',type=float,help='Generating fallback rate, default5.0.')
    ...
    evaluate = commands.add_parser('evaluate',help='Evaluate a completed simulated run against cached truth.')
    ...
    if args.config is not None:
        with args.config.open('rb') as handle:config=tomllib.load(handle)
    cores=_setting(args,config,'run','cores',len(os.sched_getaffinity(0)))
    if cores=='auto':cores=len(os.sched_getaffinity(0))
    cores=int(cores)
    if not 1<=cores<=len(os.sched_getaffinity(0)):parser.error('--cores must fit the current CPU affinity')
    try:
    ...
    if args.command!='recombination':
        ...
        if contigs:os.environ['HAPLOTYPES_CONTIGS']=json.dumps(contigs)
        vcf=_path(_setting(args,config,'inputs','vcf'))
        if vcf:os.environ['HAPLOTYPES_VCF']=vcf
    if args.command=='simulate':
        if process_contigs is not None:os.environ['BHD_SIM_CONTIGS']=','.join(process_contigs)
        if stop_after_stage is not None:os.environ['BHD_SIM_STOP_AFTER_STAGE']=stop_after_stage
        ...
        if templates:os.environ['HAPLOTYPES_TEMPLATES']=templates
        generating_map=_path(_setting(args,config,'simulation','generating_map'))
        if generating_map:os.environ['BHD_SIMULATION_RECOMBINATION_MAP']=generating_map
        else:os.environ.pop('BHD_SIMULATION_RECOMBINATION_MAP',None)
        ...
        if metadata:os.environ['HAPLOTYPES_METADATA']=metadata
